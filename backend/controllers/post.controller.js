...
export const deletePost = async (req, res) => {
  try {
    ...
    // check if post author's username matches this user's username
...
export const updatePost = async (req, res) => {
  ...
  try {
    ...
    if (post.user.toString() !== req.user._id.toString()) {
      return res
        .status(401)
        .json({ error: "You cannot edit someone else's post." });
    }

    if (newImg) {
      // destroy the post's old image in cloudinary
      if (post.img) {
        const imgId = post.img.split("/").pop().split(".")[0];
        await cloudinary.uploader.destroy(imgId);
      }
...
export const getAllPosts = async (req, res) => {
  ...
  try {
    posts = await Post.find().sort({ createdAt: -1 }).populate({
      // populate allows us to get the user object with its id
      path: "user",
      select: "-password",
    });
